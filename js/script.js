document.addEventListener("DOMContentLoaded",()=>{
  ...
  document.querySelectorAll("[data-add]").forEach(btn=>btn.addEventListener("click",()=>{
    ...
    localStorage.setItem("jlCart",JSON.stringify(cart));
    updateCart();
    showToast(btn.dataset.add+" added to your bag.")
  }));

  const checkout=document.querySelector("#checkout");
  if(checkout)checkout.addEventListener("click",()=>showToast(
    cart.length?"Demo checkout ready — connect a payment gateway for a live store.":"Your bag is empty. Pick a pair first!"
  ));
  ...
  const filters=document.querySelectorAll(".filter"),
        cards=document.querySelectorAll(".shop-grid .product-card"),
  ...
  filters.forEach(f=>f.addEventListener("click",()=>{
    ...
    cards.forEach(c=>{
      const show=type==="all"||c.dataset.category===type;
      c.style.display=show?"":"none";
      if(show)count++;
    });

    if(result)result.textContent=count+" pairs";
  }));
  ...
  function count(){
    if(done||!counters.length)return;

    const first=counters[0];

    if(first.getBoundingClientRect().top>innerHeight*.85)return;
    ...
    counters.forEach(c=>{
      ...
      const tick=now=>{
        ...
        c.textContent=Math.floor(p*target);
